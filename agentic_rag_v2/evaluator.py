...
每个问题独立运行 agentic_rag_v2 闭环探索（子问题解决链），收集检索指标和效率指标。
"""
...
class AgenticRAGV2Evaluator(BaseEvaluator):
    """Agentic RAG v2 评估器。

    每个问题独立创建 agentic_rag_v2 实例，避免状态污染。
    支持 checkpoint 断点续跑。
    """
    ...
    def evaluate_single(self, question: NormalizedQuestion) -> dict:
        ...
        """
        from agentic_rag_v2.workflow import build_agentic_rag_v2_graph
        from agentic_rag_v2.nodes import run_agentic_rag_v2

        retriever = self._make_retriever()
        config = self._make_config(retriever)
        app = build_agentic_rag_v2_graph()
