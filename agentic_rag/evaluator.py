...
每个问题独立运行 agentic_rag 闭环探索，收集检索指标和效率指标。
"""
...
class AgenticRAGEvaluator(BaseEvaluator):
    """Agentic RAG 评估器。

    每个问题独立创建 agentic_rag 实例，避免状态污染。
    支持 checkpoint 断点续跑。
    """
        ...
        Returns:
            {
                ...
                "search_depth": int,  # 等于 total_rounds
                "search_path": dict,   # exploration_history 的序列化版本
            }
        """
